(function(window) {
  ...
  function canPlayURL(link){
    return link.includes("listen");
  }
  ...
  function loadTrack(index){
    ...
    function showTrackArt(index){
        ...
        if(artwork[index]){
            ...
            if(artwork[index] == 'assets/img/default.png'){
                $('#cover-art-big').css("background", "");
            }  else{
                $('#cover-art-big').css("background-image", "url("+ artwork[index] +")").css("background-repeat", "no-repeat").css("background-position", "center");
            }
...
//          }
        ...
        if (data.selectedIndex >= data.playlist.length) {
            ...
            var index = getNextTrack();
            loadTrack(index);
            //+1 WTF kill me what is this code?!
            showTrackArt(data.selectedIndex);
        }
    ...
    actions = {
      ...
      play: function(offsetOrEvent) {
        ...
        if (!href || href.indexOf('#') !== -1) {
          ...
          if(playlistController.data.selectedIndex < 0){
            var item = playlistController.getNext();
            setTitle(item);
